...
**Status (2026-08-18):** research for [Can Supabase be the task store?](https://github.com/ayv4zyan/Cras/issues/23). Current product lock: **Supabase is the sole backend platform** — Postgres, Auth/RLS, Data API, Realtime, Storage, and Edge Functions as needed — [Does Cras need a separate Cloudflare/Hono backend?](https://github.com/ayv4zyan/Cras/issues/25), `docs/adr/0005-supabase-platform.md`. One public Deployment serves many Operators; anyone may register with Google, web uses browser OAuth, and Android uses native Google Sign-In — [How does an Operator start their own copy of Cras?](https://github.com/ayv4zyan/Cras/issues/17). Operator isolation is locked on [How is each Operator's data isolated?](https://github.com/ayv4zyan/Cras/issues/27): every Operator-owned row carries immutable `operator_id = auth.uid()`, RLS restricts access to the caller, and composite foreign keys prevent cross-Operator relationships. The exact Data API / RPC / Edge Function split remains on [Which operations use Supabase Data API vs Edge Functions?](https://github.com/ayv4zyan/Cras/issues/26). Clients are TypeScript web + Kotlin Android ([Which client stack do we lock for web and Android?](https://github.com/ayv4zyan/Cras/issues/12)).
...
One public Deployment now serves many Operators, each with isolated data; Android retains its Outbox. Google-only Auth UX is locked on [How does an Operator start their own copy of Cras?](https://github.com/ayv4zyan/Cras/issues/17). Operator ownership/RLS is locked on [How is each Operator's data isolated?](https://github.com/ayv4zyan/Cras/issues/27): immutable `operator_id`, caller-scoped RLS, same-Operator composite foreign keys, authenticated read-only Deployment data, and isolation tests across every trusted path. Only the Data API / RPC / Edge Function boundary remains open on [Which operations use Supabase Data API vs Edge Functions?](https://github.com/ayv4zyan/Cras/issues/26).
