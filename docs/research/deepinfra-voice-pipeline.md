...
**Status (2026-08-18):** research for [What do DeepInfra Voxtral Small and Gemma 4 support for in-app STT?](https://github.com/ayv4zyan/Cras/issues/9). Product behavior is locked on [How should voice create, edit, and extract task metadata?](https://github.com/ayv4zyan/Cras/issues/8); credential and pipeline placement is locked on [How do DeepInfra credentials live on web and Android?](https://github.com/ayv4zyan/Cras/issues/18); shared configuration is locked on [Where does Operator voice config live?](https://github.com/ayv4zyan/Cras/issues/19); clock and relative-date semantics are locked on [Which clock defines Today across devices?](https://github.com/ayv4zyan/Cras/issues/20); the shared timed-plan default is locked on [Where does the Instant vs Floating default live?](https://github.com/ayv4zyan/Cras/issues/21); Voice plan-type selection and correction are locked on [How does Voice capture choose Instant vs Floating?](https://github.com/ayv4zyan/Cras/issues/22). One authenticated Supabase Edge Function holds the Deployment-wide DeepInfra secret and orchestrates both provider calls. Web and Kotlin Android clients produce mono 16 kHz PCM WAV; the function validates and forwards it without transcoding. Operators store nullable stable model keys and an optional custom extractor prompt in shared Settings; the Edge Function resolves them against the Deployment's Voice model catalog. Cras retains neither recordings nor transcripts server-side. Public usage is bounded by the per-Operator Voice allowance, Deployment circuit breaker, fail-closed accounting, and content-free Usage-security records locked on [How is shared AI usage bounded?](https://github.com/ayv4zyan/Cras/issues/29).
...
Checked 2026-08-18. Voxtral Small and Gemma 4 26B-A4B-it availability/pricing, Voxtral’s 30-minute provider transcription maximum, DeepInfra’s concurrency and billing controls, and the absence of a published DeepInfra upload-size maximum were reverified against current first-party pages. Prices and catalog tags can change; slugs below are from DeepInfra’s live `/models/list`.

## Verdict

Voxtral **Small exists on DeepInfra** as `mistralai/Voxtral-Small-24B-2507`. The working OpenWhispr stack uses **Mini**, not Small: `mistralai/Voxtral-Mini-3B-2507` + `google/gemma-4-E4B-it`. DeepInfra’s speech docs only promise **mp3/wav**; the shim exists because **WebM 500s**. An in-app client can reach the API (CORS is `*`), but Cras uses an authenticated **Supabase Edge Function** to hold the Deployment-wide secret and orchestrate both provider calls. Clients normalize recorder output to mono 16 kHz PCM WAV before upload; the function does not transcode or retain audio or transcripts. Public requests are bounded by the locked Voice allowance and Deployment circuit breaker. The OpenWhispr shim’s Gemma stage is **dictation cleanup**, not reliable title/date/time JSON. Cras does not use that cleanup stage: its second provider call is structured Task extraction, and E4B is **not** tagged for JSON mode.
...
## 6. Public usage guardrails

[How is shared AI usage bounded?](https://github.com/ayv4zyan/Cras/issues/29) locks the public-launch policy around this provider pipeline:

- A Cras Voice capture is at most **2 minutes and 4 MB**. Voxtral documents up to 30 minutes for transcription, but DeepInfra publishes no upload-size maximum; 2 minutes / 4 MB is therefore a Cras product and resource boundary, not a copied provider limit.
- The default per-Operator Voice allowance is one active request, 3 requests per rolling minute, 20 requests and 10 audio minutes per rolling 24 hours, and 300 requests and 60 audio minutes per rolling 30 days.
- The Deployment circuit breaker stops new Voice capture at $1 per rolling 24 hours or $5 per rolling 30 days. A maintainer-controlled `voice_enabled` setting can stop new captures independently. DeepInfra automatic top-up is disabled, and its provider spending limit is the final backstop.
- Server time defines every rolling window. Authentication, WAV structure, size, and duration fail before accounting. Trusted accounting atomically reserves the Operator request/audio allowance and conservative estimated spend immediately before the first provider call; accounting failure fails closed for Voice while ordinary Task features remain available.
- At most two internal retries are allowed for transient 429, timeout, connection, or 5xx failures. They do not double-charge the Operator. Provider-bound failures retain allowance and uncertain spend until reconciliation; a later client retry is a new request.
- Usage-security records contain only a keyed pseudonymous identity, time bucket, aggregate usage, provider-attempt category, model catalog key, token counts, cost, and reservation state. They never contain email, raw Google identity, Tasks, recordings, transcripts, prompts, Drafts, or provider responses. Buckets expire 35 days after their end; after account purge, the pseudonymous key remains only while an associated unexpired bucket exists.

---

## 7. Historical shim cleanup vs Cras smart metadata
...
3. Pass the **recording-start time and the recording device’s timezone**. The model has no “now,” so relative speech such as “tomorrow at 3” cannot be grounded otherwise. Resolve the expression immediately into the exact date shown in the Draft; do not retain a relative token or silently change the result after midnight.
4. Treat extracted values as **untrusted** and validate the schema. Optional fields that speech does not state remain null unless a field-specific decision defines a default or derived value. For a clock time, the extractor emits Instant/Floating only when speech explicitly names the type; it never infers type from contextual wording. Otherwise the Voice pipeline applies the effective timed-plan default when creating a timed Task or adding a time to an untimed/Date-only Task. An already timed Task preserves its existing type unless speech explicitly changes it.
