...
One public Deployment now serves many Operators, each with isolated data; Android retains its Outbox. Google-only Auth UX is locked on [How does an Operator start their own copy of Cras?](https://github.com/ayv4zyan/Cras/issues/17). Tenant ownership/RLS and the Data API / RPC / Edge Function boundary remain open on [How is each Operator's data isolated?](https://github.com/ayv4zyan/Cras/issues/27) and [Which operations use Supabase Data API vs Edge Functions?](https://github.com/ayv4zyan/Cras/issues/26).
