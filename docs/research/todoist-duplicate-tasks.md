...
That is one task whose date shifts on complete — not two dated rows that the app collapses. Recurrence and a separate reminder field remain excluded from the current MVP Task model by [What is a Task, and what are Inbox, Today, and Upcoming?](https://github.com/ayv4zyan/Cras/issues/6). A later refinement makes every timed plan automatically request a best-effort Notification while Date-only does not — [Do notifications belong in Cras MVP?](https://github.com/ayv4zyan/Cras/issues/30). Neither decision answers the forgotten second *one-off* create.
