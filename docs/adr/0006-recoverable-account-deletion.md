...
## Later Notification boundary

Automatic Notifications and eligible installations were added after this ADR. This ADR does not silently decide whether Pending deletion suppresses scheduled Notifications, whether recovery reschedules future Notifications, or when delivery jobs and installation endpoints are deleted. [Which account-lifecycle operations use Edge Functions?](https://github.com/ayv4zyan/Cras/issues/31) owns those lifecycle semantics. [How are Notifications scheduled and delivered on Android and web?](https://github.com/ayv4zyan/Cras/issues/34) is blocked by that decision and owns only the delivery machinery that implements it. Retaining Tasks during the Recovery window must not be read as permission to continue Notification delivery.
